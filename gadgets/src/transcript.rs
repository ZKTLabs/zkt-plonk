...
use plonk_core::{commitment::KZG10Commitment, transcript::TranscriptProtocol};


pub struct EthereumTranscript {
    state_0: H256,
    state_1: H256,
    counter: u32,
}

impl EthereumTranscript {
    const DST_0: u8 = 0;
    const DST_1: u8 = 1;
    const DST_CHALLENGE: u8 = 2;

    fn append_bytes_without_label(&mut self, item: &[u8]) {
        let old_state = self.state_0;
        let new_state = self.state_1;

        let mut data = Vec::with_capacity(1 + 32 + 32 + item.len());
        data.push(Self::DST_0);
        data.extend_from_slice(old_state.as_bytes());
        data.extend_from_slice(new_state.as_bytes());
        data.extend_from_slice(item);
        self.state_0.as_mut().copy_from_slice(&Keccak256::digest(&data)[..32]);

        data.clear();
        data.push(Self::DST_1);
        data.extend_from_slice(old_state.as_bytes());
        data.extend_from_slice(new_state.as_bytes());
        data.extend_from_slice(item);
...
impl TranscriptProtocol<Fr, KZG10Commitment<Bn254>> for EthereumTranscript {
    ...
    fn append_scalars<'a, I>(&mut self, label: &'static str, items: I)
        where I: IntoIterator<Item=&'a Fr>,
    {
        for item in items {
    ...
    fn append_commitments<'a, I>(&mut self, label: &'static str, items: I)
        where I: IntoIterator<Item=&'a KZG10Commitment<Bn254>>,
    {
        for item in items {
            self.append_commitment(label, item);
        }
    }

    fn challenge_scalar(&mut self, _label: &'static str) -> Fr {
        let mut data = Vec::with_capacity(1 + 32 + 32 + 4);
        data.push(Self::DST_CHALLENGE);
...
mod test {
    ...
    #[test]
    fn test_transcript() {
        let mut transcript = EthereumTranscript::new("test");
